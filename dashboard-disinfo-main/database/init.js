...
class Database {
        ...
        return new Promise((resolve, reject) => {
            this.db.run('DELETE FROM posts', (err) => {
                if (err) {
                    ...
                    console.log('🗑️  Base de datos limpiada para simulador');
                    resolve();
                }
            });
        });
    ...
    async clearDataBySource(source) {
        return new Promise((resolve, reject) => {
            this.db.run('DELETE FROM posts WHERE source = ?', [source], (err) => {
                if (err) {
                    console.error(`Error clearing data for source ${source}:`, err);
                    reject(err);
                } else {
                    console.log(`🗑️  Datos de fuente '${source}' limpiados de la base de datos`);
                    resolve();
                }
            });
        });
